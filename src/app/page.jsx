...
import CourageCelebration from "@/components/courage-celebration"
...
export default function Home() {
  ...
  useEffect(() => {
    const generated = Array.from({ length: 20 }).map(() => ({
      ...
      top: `${Math.random() * 100}%`,
      color: ["bg-indigo-300", "bg-purple-300", "bg-yellow-300", "bg-violet-300", "bg-blue-300"][Math.floor(Math.random() * 3)],
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-indigo-100 to-purple-100 flex flex-col items-center justify-center p-4 overflow-hidden">
      {isBirthday && <Confetti />}
      <FloatingHearts />
      ...
      >
        <motion.div className="bg-white bg-opacity-80 backdrop-blur-sm rounded-3xl shadow-xl shadow-indigo-100 p-8 border-2 border-indigo-200"
          initial={{ scale: 0.9 }}
          animate={{ scale: 1 }}>
          <AnimatePresence mode="wait">
            {isBirthday ? (
              <CourageCelebration key="celebration" />
            ) : (
      ...
      >
        <motion.button
          ...
          className="bg-gradient-to-r z-10 from-indigo-500 to-purple-500 shadow-lg hover:shadow-xl transition-all rounded-full font-medium text-white py-4 px-8 cursor-pointer border-2 border-white flex items-center gap-3"
          whileTap={{ scale: 0.95 }}
          animate={{
            y: [0, -5, 0],
            scale: [1, 1.03, 1],
